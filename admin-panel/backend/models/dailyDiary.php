...
class DailyDiary {
    ...
    public function create($title, $description, $category, $priority, $class_range, $staff_instruction, $event_date, $event_time, $file){
        $stmt = $this->conn->prepare(
            ...
            (title, description, category, priority, class_range, staff_instructions, event_date, event_time, file_path)
            VALUE (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
    ...
    public function update($id, $title, $description, $category, $priority, $class_range, $staff_instruction, $event_date, $event_time, $file) {
        ...
        $stmt = $this->conn->prepare($query);
        $stmt->bind_param("ssssssssi", $title, $description, $category, $priority, $class_range, $staff_instruction, $event_date, $event_time, $file, $id);
        return $stmt->execute();
    }
